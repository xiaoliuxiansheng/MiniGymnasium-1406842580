...
module.exports = Behavior({
	...
	methods: {
		...
		onLoad: function (options) {
			this.setData({
				canOrder: options?.canNotOrder ? false : true // 是否可以进入预约界面
			})
		...
		_loadDetail: async function () {
			...
			let params = {
				id,
				isCoach: !this.data.canOrder // 是否显示签到数据
			};
			let opt = {
				title: 'bar'
			};
		...
		// 签到
		bindSignIn: async function(e) {
			if (this.data.canOrder) return
			let mark = pageHelper.dataset(e, 'mark');
			let id = pageHelper.dataset(e, 'id');
			let opts = {
				title: '提交中'
			}
			await cloudHelper.callCloudSumbit('meet/coach_sign_in', {id, mark}, opts).then(res => {
				const {meet} = this.data
				const dayIdx = meet.MEET_DAYS_SET.findIndex(day => day._id === id)
				const timeIdx = meet.MEET_DAYS_SET[dayIdx].times.findIndex(time => time.mark === mark)
				meet.MEET_DAYS_SET[dayIdx].times[timeIdx].isSignIn = true
				meet.MEET_DAYS_SET[dayIdx].times[timeIdx].signStatus = 1
				wx.showToast({
					title: '签到成功！',
					icon: 'success'
				});
