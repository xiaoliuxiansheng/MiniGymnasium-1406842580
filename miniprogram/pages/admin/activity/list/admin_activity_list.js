...
Page({
	...
	bindStatusTap: async function (e) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/activity_list', params).then(res => {
		...
		wx.showModal({
			...
			success: async (res) => {
			  if (res.confirm) {
				...
				await cloudHelper.callCloudSumbit('admin/activity_del', {id: e.currentTarget.dataset.id});
				pageHelper.delListNode(e.currentTarget.dataset.id, this.data.dataList.list, '_id');
				this.data.dataList.total--;
				this.setData({
					dataList: this.data.dataList
				});
