...
Page({
	...
	getCoachList: async function() {
		const list = await cloudHelper.callCloudSumbit('admin/user_list_by_type', {type: 2});
		this.setData({
			coachOptions: list.data.map(item => ({label: item.USER_NAME, val: item.USER_MINI_OPENID}))
		})
	},
	_loadDetail: async function () {
		...
		pageHelper.formSetBarTitleByAddEdit(id, '后台-场地/预约');
		...
		this.setData({
			...
			formPrice: meet.MEET_PRICE,

			formPayType: meet.MEET_PAY_TYPE,
			formClassType: meet.MEET_CLASS_TYPE
		});
	},
	...
	bindFormAddSubmit: async function () {
		...
		if(!data.formPayType) return pageHelper.formHint(this, 'formPayType', '请选择「消费方式」');
		if(!data.formPrice && data.formTypeId === '2') return pageHelper.formHint(this, 'formPrice', `请输入「消费${data.formPayType === 1 ? '积分' : '金额'}」`);
		...
		if(data.formTypeId === '2' && !data.formCoachId) return pageHelper.formHint(this, 'formCoachId', '请选择「教练」');
		
		if(data.formTypeId === '2' && !data.formClassType) return pageHelper.formHint(this, 'formClassType', '请选择「课程类型」');

		if (data.formTypeId === '1') {
			data.formDaysSet = []
			data.formFormSet = []
		}
		...
		if (data.formDaysSet.length <= 0 && data.formTypeId === '2') {
			pageHelper.anchor('formDaysSet', this);
			return pageHelper.formHint(this, 'formDaysSet', '请配置「可预约时段」');
		}
		if (data.formFormSet.length <= 0 && data.formTypeId === '2') return pageHelper.showModal('请至少设置一项「用户填写资料」');
