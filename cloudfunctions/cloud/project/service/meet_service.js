...
class MeetService extends BaseService {
	...
	async getDaysSet(meetId, startDay, endDay = null) {
		...
		let list = await DayModel.getAllBig(where, 'day,dayDesc,times, _id', orderBy, 10000);
	...
	async viewMeet(meetId, isCoach) {
		...
		let getDaysSet = [];
		meet.MEET_DAYS_SET = await this.getDaysSet(meetId, isCoach ? '2022-09-01': timeUtil.time('Y-M-D')); //今天及以后
		...
		if (isCoach) {
			...
			//signStatus 1 已签到 2 待签到 3 未签到 4 未到签到时间
			getDaysSet.forEach(day => {
				day.times.forEach(time => {
					if (!time.isSignIn) {
						const timeStamp = timeUtil.time2Timestamp(day.day + ' ' + time.start)
						if (nowStamp > timeStamp) {
							time.signStatus = 3
						} else {
							if ((timeStamp - nowStamp) < (config.CAN_SIGN_IN_TIME * 1000)) {
								time.signStatus = 2
							} else {
								time.signStatus = 4
							}
		...
		ret.MEET_MEET_TYPE_ID = meet.MEET_TYPE_ID
		ret.MEET_PAY_TYPE = meet.MEET_PAY_TYPE
		ret.MEET_PRICE = meet.MEET_PRICE

		// 判断预约是否是教练预约 则返回教练信息
		if (meet.MEET_TYPE_ID === '2' && meet.MEET_COACH_ID) {
			let where = {
				USER_MINI_OPENID: meet.MEET_COACH_ID
			}
			let fields = 'USER_MOBILE,USER_TRADE,USER_NAME, USER_COACH_TYPE';
			ret.coachInfo = await UserModel.getOne(where, fields)
		}
	...
	async detailForJoin(userId, meetId, timeMark) {
		...
		let fields = 'MEET_DAYS_SET,MEET_FORM_SET, MEET_TITLE, MEET_PAY_TYPE, MEET_PRICE';
	...
	/** 按天获取预约项目 */
	async getMeetListByDay(day, typeId, coachId, userId) {
		let where = {
			MEET_STATUS: MeetModel.STATUS.COMM,
		};

		if (typeId) {
			where.MEET_TYPE_ID = typeId
		}

		if (coachId) {
			where.MEET_COACH_ID = coachId
		}

		const {USER_CLASS_TYPE} = await UserModel.getOne({USER_MINI_OPENID: userId})

		if (USER_CLASS_TYPE && !coachId){
			where.MEET_CLASS_TYPE = USER_CLASS_TYPE
		}
		let orderBy = {
			'MEET_ORDER': 'asc',
			'MEET_ADD_TIME': 'desc'
	...
	/** 获取从某天开始可预约的日期 */
	async getHasDaysFromDay(day, typeId, coachId) {
		// return []
		let where = {
			MEET_TYPE_ID: '2'
		}
		...
		let meetWhere = {}
		if (typeId) {
			meetWhere.MEET_TYPE_ID = typeId
		}
		if (coachId) {
			meetWhere.MEET_COACH_ID = coachId
		}

		if (typeId || coachId) {
			let fields = '_id';
			let list = await MeetModel.getAll(meetWhere, fields);
			meetOptions = list.map(item => item._id)
			where.DAY_MEET_ID = ['in', meetOptions]
		}
	...
	async getMeetList({
		...
		let fields = 'MEET_TITLE,MEET_STYLE_SET,MEET_DAYS,MEET_COACH_ID, MEET_TYPE_ID';
		...
		if (typeId) {
			...
			if (typeId === '2') {
				const {USER_CLASS_TYPE, USER_TYPE} = await UserModel.getOne({USER_MINI_OPENID: userId})
				if (USER_CLASS_TYPE && USER_TYPE == 1){
					where.MEET_CLASS_TYPE = USER_CLASS_TYPE
				}
			}
		}
		...
		if (placeId) {
			...
			const users = await UserModel.getAll(whereUser, fields)
			where.MEET_COACH_ID = ['in', users.map(user => user.USER_MINI_OPENID)]
		}
	...
	async cancelMyJoin(userId, joinId) {
		...
		if (meet.MEET_PAY_TYPE === 1) {
			const passport = new PassportService()
			await passport.updateCanuseCount(userId, meet.MEET_PRICE, 'add')
		}
	...
	/** 教练签到 */
	async signIn(id, mark) {
		const {times} = await DayModel.getOne({_id: id})
		const markIdx = times.findIndex(item => item.mark === mark)
		await DayModel.edit({_id: id}, {['times.' + markIdx + '.isSignIn']: true})
		return 
	}
}
